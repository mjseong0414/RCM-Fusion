...
def create_nuscenes_infos(root_path,
    ...
    if test:
        ...
        info_path = osp.join(out_path,
                             '{}_infos_train_sweep6_real.pkl'.format(info_prefix))
        mmcv.dump(data, info_path)
        data['infos'] = val_nusc_infos
        # info_val_path = osp.join(out_path,
        #                          '{}_infos_temporal_radar_val_sweep6.pkl'.format(info_prefix))
        info_val_path = osp.join(out_path,
                                 '{}_infos_val_sweep6_real.pkl'.format(info_prefix))
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        # if sample['token'] != 'c9ff2ad28a904840b4cec1ebadb7bf2d':
        #     continue
        ...
        for radar in radar_types:
            ...
            # while len(RADAR_sweeps) < 5*max_sweeps:
            while len(sweeps_bucket) < max_sweeps:
                if not sd_rec['prev'] == '':
                    RADAR_sweep = obtain_sensor2top(nusc, sd_rec['prev'], l2e_t,
                                                l2e_r_mat, e2g_t, e2g_r_mat, radar)
                    RADAR_sweep['l2e_t']=l2e_t
                    RADAR_sweep['l2e_r_mat']=l2e_r_mat
                    RADAR_sweep['e2g_t']=e2g_t
                    RADAR_sweep['e2g_r_mat']=e2g_r_mat
                    sd_rec =nusc.get('sample_data', sd_rec['prev'])
                    RADAR_sweeps.append(RADAR_sweep)
                    sweeps_bucket.append(RADAR_sweep)
                else:
                    break
...
def generate_record(ann_rec: dict, x1: float, y1: float, x2: float, y2: float,
    ...
    # coco_rec['bbox'] = [x1, y1, x2 - x1, y2 - y1] ############################## original code
...
def make_multisweep_radar_data(nusc, sample, max_sweeps, root_path,info):
    ...
    for radar_channel in radar_channels:
        ...
        radar_timestamp[radar_channel] = ref_radar_time # for time gap on mmdetection3d
        ...
        for _ in range(max_sweeps):
            ...
            current_pc = RadarPointCloud.from_file(radar_path).points.T # invalid_states, dynprop_states, ambig_states
            current_pc[:,12] = time_gap
            current_pc = point_filtering(current_pc)
            # print(current_pc.shape)
    ...
    info['radar_timestamp'] = radar_timestamp
